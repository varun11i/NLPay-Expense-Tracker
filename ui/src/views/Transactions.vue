        ...
        <div class="card-body p-3">
          ...
          <div class="flex justify-between items-center mt-3 pt-3 border-t border-base-200">
            ...
            <div class="flex items-center gap-2">
              <!-- Confirm-all dropdown + button -->
              <div class="flex items-center gap-2">
                <select v-model="confirmScope" class="select select-bordered select-xs">
                  <option value="all">Confirm all</option>
                  <option value="visible">Confirm visible</option>
                  <option value="today">Confirm today</option>
                </select>
                <button
                ...
                >
                  {{ isBulkConfirming ? 'Confirming...' : 'Apply' }}
                </button>
              </div>
      ...
      <div class="modal-box max-w-lg">
        ...
        <form @submit.prevent="addExpense">
          <textarea
            ...
            placeholder="e.g., Paid $15 for lunch at Subway"
          ...
          <div class="grid grid-cols-2 gap-2 mt-2">
            <div>
              ...
              <input
                type="date"
                v-model="newExpense.transaction_date"
                class="input input-bordered input-sm w-full"
                required
                :disabled="isSubmitting"
              />
          ...
          <p class="text-xs text-base-content/60 mt-2">
            AI will parse amount, category, currency, and the date you selected.
          </p>
        ...
        <form @submit.prevent="saveBulkEdit" class="space-y-3">
          ...
          <div class="modal-action mt-3">
            ...
            <button
              ...
              :disabled="isBulkEditing || (!bulkEditForm.category && bulkEditForm.confirm === null)"
            >
...
const confirmScope = ref<'all' | 'visible' | 'today'>('all')
...
const handleConfirmAll = async () => {
  ...
  if (confirmScope.value === 'all') {
    targetTxs = transactions.value
  } else if (confirmScope.value === 'visible') {
    targetTxs = filteredTransactions.value
  } else if (confirmScope.value === 'today') {
    const today = new Date().toISOString().split('T')[0]
    targetTxs = transactions.value.filter(
      t => t.transaction_date.split('T')[0] === today
    )
  }
  ...
  try {
    for (const t of targetTxs) {
      if (!t.confirm) {
        await updateTransaction(t.id, { confirm: true })
      }
    }
    clearSelection()
    loadFilteredTransactions()
